...
class VFS {
	...
	_getHookedTextStream(filename, content, unicode, iomode) {
		let self = this;
        let textFile = new TextStream(filename, content, unicode, iomode);
    ...
    createTextFile(filename, overwrite, unicode) {
        ...
        // Making sure we have the right filename
        filename = resolvedFile.name;

		return this._getHookedTextStream(filename, '', unicode, 2);
    }
    ...
    openTextFile(filename, iomode, create, format) {
        ...
        if (this.fileExists(filename) === 0 && create === true) {
            ...
            return this._getHookedTextStream(origFile.Name, origFile._content, true, iomode);
        }
    }
}
