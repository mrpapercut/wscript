...
class VFS {
    ...
    _getSubfiles(path) {
        ...
        return this._vfs.filter(entry => {
            return entry.type === 'file' && entry.path.replace('\\' + entry.name, '') === path;
        });
    }
    ...
    copyFolder(folder, destination) {
        ...
        subfolders.forEach(folder => this.copyFolder(folder.path, [destination, folder.name].join('\\')));
        subfiles.forEach(file => this.copyFile(file.path, [destination, file.name].join('\\')));
    }
